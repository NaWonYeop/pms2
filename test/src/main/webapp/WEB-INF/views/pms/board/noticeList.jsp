				...
				<table id="data_list" class="table table-bordered">
					...
					<tbody>
						<c:forEach var="noti" items="${notice }">
							<tr>
								<c:choose>
									<c:when test="${fn:contains(noti.brd_ntc_prop,'1')}">
										<!-- 	<div class="table-row"
								...
								<c:choose>
									...
									<c:otherwise>
										<td class="text-center">${noti.brd_id }</td>
									</c:otherwise>
								</c:choose>
								...
								<td>
									<div class="">
										<a href="freeboardSelect?brd_id=${noti.brd_id }">${noti.brd_ttl }</a>
									</div>
								</td>
								<td>
									<div class="text-center">${noti.user_name }</div>
								</td>
								<td>
									<div class="text-center">
										<fmt:formatDate value="${noti.brd_reg_date }"
											pattern="yyyy-MM-dd" />
...
if (user == null) {%>
	...
	<%} else {%>
		location.href = "projectboardInsertForm";
	<%}%>
		;
		}
		jQuery(function($) {
