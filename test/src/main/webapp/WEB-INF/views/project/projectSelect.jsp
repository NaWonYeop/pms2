	...
	<section class="course_details_area section_padding">
		<div class="container">
				...
				<div class="row">
					...
					<c:if test="${project.prj_fnd_prop == 1}">
						<div class="col-4">
							...
							<input type="hidden" id="dol"> <input type="hidden"
								id="etherDol"> <input type="hidden" id="etherWon">
						</div>
					</c:if>
				</div>
			</div>
				...
				<div class="row">
					...
					<div class="col-4">
						<sec:authorize access="isAuthenticated()">
							<button type="button" id="heartbtn" class="btn_4"
								style="background-color: #F27457;" onclick="heart()">찜하기</button>
							<button type="button" id="heartCancelbtn" class="btn_4"
								style="background-color: #F27457;" onclick="heartCancel()">찜취소</button>
							<form style="display: inline-block;" id="ajaxAppPrjInsertForm"
								onsubmit="return false" onclick="ajaxAppPrjInsertForm()">
								...
								<input type="hidden" name="master_id"
									...
									name="user_id" value="${sessionScope.sessionUser.user_id }">
								<input type="submit" class=" btn_4"
									style="background-color: #F27457; display: inline-block;"
									value="참가신청">
							</form>
						</sec:authorize>
					</div>
			...
			<div class="row">
				<div class="col-lg-8 course_details_left">
					...
					<c:if
						...
						<form style="display: inline-block;" action="rewardInsertForm">
							...
							<input type="hidden" name="master_id"
								value="${project.master_id}"> <input type="hidden"
								id="go" name="go" value="selectPage"> <input
								type="submit" class="rfnd btn_4" value="리워드 관리">
						</form>
				...
				<input type="hidden" id="buyMaxCoinCount" name="buyMaxCoinCount"
					value="${buyMaxCoinCount }"> <input type="hidden"
					...
					type="hidden" id="ether_id" name="ether_id"
					value="${sessionScope.sessionUser.ether_id }"> <input
					type="hidden" id="master_id" name="master_id"
					value="${project.master_id}"> <input type="hidden"
					id="masterAcc" value="${masterAcc}">
						...
						<form>
							<article class="blog_item">
								<div class="blog_details">
									...
									ETHER
									<h2>${ String.format("%.6f",(reward.rwd_prc / Rether))}</h2>
									<input type="hidden" id="e${reward.reward_id }"
										value="${reward.rwd_prc / Rether}"> 내용 :
									${reward.rwd_cnt}<br> 재고수량 :
									${reward.rwd_goal-reward.rwd_cot}<br>
									<%-- 판매 여부 : ${reward.rwd_stt}<br> --%>
									<input type="number" id="c${reward.reward_id }"
										name="buy_count" required placeholder="구매수량(기본:1개)">
									<div class="btns">
										<!-- 버튼들은 클래스 바꾸면 안되요 -->
										<input type="hidden" name="reward_id"
											value="${reward.reward_id }"> <input type="button"
											class="wBuy btn_4" id="${reward.reward_id }" value="현금구매"
											style="background-color: #F27457;"> <input
											type="button" class="rfnd btn_4" id="${reward.reward_id }"
											value="코인구매" style="background-color: #F27457;">
									</div>
								</div>
							</article>
						</form>
		...
		<div class="modaldal content">
			<div class="content modalcontent">
				<div class="justify-content-center">
					<h2 class="content title" style="text-align: center;">결제금액
						단위(wei)</h2>
					<br> <br>
	...
	$(document).ready(function(){
			...
			$.ajax({
		        ...
		        success: function(result){
					...
					$("#sise").html("현금구입 달러시세 : " + dol.toString()
							  .replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")+"<br>"
							+"1이더 달러시세 : " + etherDol.toString()
							  .replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")+"<br>"
							+"1이더 원화시세 : " + etherWon.toString()
							  .replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")+"<br>");
		        },
		        error:function(){
		          console.log("Insert ajax 통신 실패!!!");
